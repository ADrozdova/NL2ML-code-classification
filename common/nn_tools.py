...
parser.add_argument("N_TRIALS", help="number of optuna trials", type=int)
...
DEFAULT_MASKING_RATE = 0.9978617998679711
...
def select_hyperparams(df, n_classes, model_path):
    ...
    model_params, history = None, None
    ...
    if N_TRIALS > 0:
        ...
        DEFAULT_HYPERPARAMS['n_classes'] = n_classes
        model, history = train_new_model(df_train, df_test, N_EPOCHS, DEFAULT_HYPERPARAMS, DEFAULT_MASKING_RATE)

        checkpoint_path = CHECKPOINT_PATH_TEMPLATE.format(0)
        history_path = LEARNING_HISTORY_PATH_TEMPLATE.format(0)
        torch.save(model.state_dict(), checkpoint_path)
        pickle.dump(history, open(history_path, "wb"))
...
if __name__ == "__main__":
    ...
    print("hyperparams:", model_params)
